...
namespace CaféPourLaVie.Services
{
    public class ReportService : IReportService
    {
        ...
        public async Task<ReportViewModel> GetReportAsync(DateTime fromDate, DateTime toDate)
        {
            ...
            // =========== PRODUCT SALES ===========
            var productSales = await _context.OrderDetails.Where(od => od.Order.Status == OrderStatus.Completed &&
            ...
            // ============ REVENUE BY DATE ============
            var revenueByDate = await _context.Orders.Where(o => o.OrderDate >= startDate &&
                                                                 o.OrderDate < endDate &&
                                                                 o.Status == OrderStatus.Completed)
            ...
            var lowStockProducts = await _context.Products.CountAsync(p => p.Quantity < 10);


            // inventory value = sum of (quantity * price)
            var inventoryValue = await _context.Products
                                               .SumAsync(p => (decimal?)(p.Quantity * p.Price)) ?? 0;


            // ============== IMPORT RECEIPTS ==============
            var importReceipts = _context.ImportReceipts.Where(r =>r.ImportDate >= startDate &&
                                                                    r.ImportDate < endDate);


            var totalImportReceipts = await importReceipts.CountAsync();


            var totalImportValue = await importReceipts.SumAsync(r => (decimal?)r.TotalAmount) ?? 0;
